...
import {
  getMetadata,
  getDelayTime
} from "../section-metadata/section-metadata.js";
import {
  getGnavHeight
...
function promoIntersectObserve(el, stickySectionEl, options = {}) {
  const io = new IntersectionObserver((entries, observer) => {
    entries.forEach(entry => {
      ...
      const abovePromoStart = entry.target === stickySectionEl && entry.isIntersecting || stickySectionEl?.getBoundingClientRect().y > 0;
...
function handleStickyPromobar(section, delay) {
  ...
  if (section.querySelector(':is(.promobar, .notification)').classList.contains('no-delay') || delay && section.classList.contains('popup')) {
    hasScrollControl = true;
    section.classList.remove('hide-sticky-section');
  }
  if (!hasScrollControl && main.children[0] !== section) {
    stickySectionEl = createTag('div', {
      class: 'section show-sticky-section'
    });
      ...
      switch (sticky) {
        ...
        case 'sticky-bottom': {
          if (section.querySelector(':is(.promobar, .notification)')) {
            ...
            const delay = getDelayTime(metadata.delay?.text);
            if (delay) setTimeout(() => {
              handleStickyPromobar(section, delay);
            }, delay);
            else handleStickyPromobar(section, delay);
          }
          main.append(section);
